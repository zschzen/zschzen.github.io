...
float text( in vec2 uv )
{
    ...
    vec2 charSize = vec2(.05, .0575);
    // and a starting position.
    vec2 charPos = vec2(0.95, 0.90);
    // Draw some text!
    float chr = 0.0;
    // Bitmap text rendering!
    chr += drawChar( CH_R, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_O, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_T, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_BLNK, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_PLUS, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_BLNK, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_T, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_R, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_A, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_N, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_S, charPos, charSize, uv); charPos.x += .06;
    chr += drawChar( CH_L, charPos, charSize, uv); charPos.x += .06;
...
void drawScene( in vec2 uv, in float txt, out vec3 outColor )
{
    outColor = vec3(.0);
    float d = sdBox(uv, vec2(0.1, 0.15));

    outColor = d < .0 ? NEGATIVE_BG : POSITIVE_BG;
    //outColor *= vec3(1.0) - vec3(smoothstep(0.0, 0.01, d));
    outColor *= 1.0 - exp(-10.0 * abs(d));
    outColor *= 0.8 + 0.2 * cos(300.0 * d);
    outColor = mix( outColor, vec3(1.), 1.0 - smoothstep(0.0, 0.01, abs(d)));
    outColor = mix( outColor, vec3( 0.9450, 0.9803, 0.5490 ), smoothstep(0.0, 0.01, (txt)) );
}

void main()
{
    // Normalizar coordenadas do fragmento
    vec2 uv = gl_FragCoord.xy;
    uv = (2.0 * uv.xy - u_resolution.xy) / u_resolution.x;
    uv.x = mod(uv.x, 1.0) - .5;
    ...
    {
        float txt = text2(gl_FragCoord.xy / u_resolution.y);

        uv -= 0.1 + 0.1 * sin( u_time );
        uv *= rot2D( u_time );
        drawScene( uv, txt, color );
    }
    else
    {
        float txt = text( gl_FragCoord.xy / u_resolution.y );
    ...
    color = mix( color, vec3(0.), smoothstep(0.0, 0.1, 1.0-(lineSegment(gl_FragCoord.xy / u_resolution.xy, startPoint, endPoint))));
