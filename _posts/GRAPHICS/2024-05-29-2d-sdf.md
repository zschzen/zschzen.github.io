...
tags: [2D SDF, Signed Distance Field, 2D graphics, Portuguese]
math: true
image:
...
Tudo o que abordarei aqui não é conhecimento novo, aprendi a maior parte lendo os artigos de [Ronja] e [Inigo Quilez], que recomendo fortemente para obter um melhor entendimento.
...
Portanto, a notação `SDF(p)` que comumente representa a função de distância assinada de um ponto `p` de tal modo que:
